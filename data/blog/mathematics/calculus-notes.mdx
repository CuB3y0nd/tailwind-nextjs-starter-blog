...
**Lagrange's notation:** $\displaystyle f^{\prime }$

**Leibniz's notation:** $\displaystyle \frac{dy}{dx}$

**Newton's notation:** $\displaystyle \dot{y}$
...
every\ point\ over\ ( a,\ b) \ is\ differentiable
\end{cases} \Longrightarrow \ there\ exists\ some\ c\in ( a,\ b) \ where\ \frac{\Delta y}{\Delta x} =\frac{f( b) -f( a)}{b-a} =f^{\prime }( c)
$$
...
$$
critical\ points\ exists\ when\ non\ endpoint\ point\ at\ x=a\ \begin{cases}
f^{\prime }( a) =0\\
f^{\prime }( a) \ undefined
\end{cases}
$$
...
Let $f$ be a continuous real−valued function defined on $[ a,\ b]$. Let $F$ be the function defined, for all $x$ in $[ a,\ b]$, by $\displaystyle F( x) =\int _{a}^{x} f( t) dt$
